...
<div style="background: #fff; padding: 30px; border-radius: 10px; max-width: 800px;">
    <form action="{{ route('admin.services.store') }}" method="POST">
        ...
        <div class="form-group">
            <label>Title</label>
            <input type="text" name="title" class="form-control" required placeholder="e.g. Cardiology">
        </div>
        <div class="form-group">
            <label>Icon</label>
            <div style="display: flex; align-items: center; gap: 15px;">
                <select name="icon" id="iconSelect" class="form-control" style="flex: 1;" onchange="updateIconPreview()">
                    ...
                    @foreach($icons as $icon)
                        <option value="{{ $icon }}">{{ str_replace(['fas fa-', '-'], ['', ' '], $icon) }}</option>
                    @endforeach
                </select>
        ...
        <script>
            function updateIconPreview() {
                ...
                if(icon) {
                    ...
                    preview.innerHTML = '<i class="fas fa-notes-medical"></i>';
                }
            }
        </script>
        <div class="form-group">
            <label>Description</label>
            <textarea name="description" class="form-control" rows="4" required placeholder="Detailed description of the service..."></textarea>
        </div>
